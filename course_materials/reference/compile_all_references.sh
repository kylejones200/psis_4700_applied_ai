...
# Compile All Reference Materials to PDF
# This script compiles all .tex files in the reference directory to PDF format
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}PSIS 4700: Applied AI Reference Compiler${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
echo -e "Found ${YELLOW}${TOTAL}${NC} reference material(s) to compile"
echo ""
...
if [ $SUCCESS -eq $TOTAL ]; then
    echo -e "${GREEN}All reference materials compiled successfully!${NC}"
    echo -e "PDFs saved to: ${YELLOW}pdfs/${NC}"
    exit 0
else
    echo -e "${YELLOW}Some reference materials failed to compile. Check log files for details.${NC}"
    exit 1
fi
